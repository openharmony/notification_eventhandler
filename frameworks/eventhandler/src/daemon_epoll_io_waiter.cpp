...
#include "daemon_epoll_io_waiter.h"
...
DEFINE_EH_HILOG_LABEL("DaemonIoWaiter");
...
DaemonIoWaiter& DaemonIoWaiter::GetInstance()
{
    static DaemonIoWaiter epollIoWaiter;
    return epollIoWaiter;
}

bool DaemonIoWaiter::Init()
{
    HILOGD("enter");
...
void DaemonIoWaiter::StartEpollIoWaiter()
{
    if (running_.exchange(true)) {
        return;
    }
    auto task = std::bind(&DaemonIoWaiter::EpollWaitFor, this);
    epollThread_ = std::make_unique<std::thread>(task);
}

void DaemonIoWaiter::StopEpollIoWaiter()
{
    isFinished_ = true;
    NotifyAll();
...
void DaemonIoWaiter::HandleFileDescriptorEvent(int32_t fileDescriptor, uint32_t events)
{
...
void DaemonIoWaiter::HandleEpollEvents(struct epoll_event *epollEvents, int32_t eventsCount)
{
...
void DaemonIoWaiter::EpollWaitFor()
{
    if (epollFd_ < 0) {
...
bool DaemonIoWaiter::WaitFor(std::unique_lock<std::mutex> &lock, int64_t nanoseconds)
{
    return true;
}

void DaemonIoWaiter::NotifyOne()
{
    // Epoll only support wake up all waiting thread.
    NotifyAll();
}

void DaemonIoWaiter::NotifyAll()
{
    if (awakenFd_ < 0) {
...
bool DaemonIoWaiter::SupportListeningFileDescriptor() const
{
    return true;
}

bool DaemonIoWaiter::AddFileDescriptor(int32_t fileDescriptor, uint32_t events, const std::string &taskName)
{
    return true;
}

bool DaemonIoWaiter::AddFileDescriptorInfo(int32_t fileDescriptor, uint32_t events, const std::string &taskName,
    const std::shared_ptr<FileDescriptorListener>& listener, EventQueue::Priority priority)
{
    ...
    HILOGD("DaemonIoWaiter add file %{public}d, %{public}s, %{public}d", fileDescriptor, taskName.c_str(), priority);
    return true;
}

void DaemonIoWaiter::RemoveFileDescriptor(int32_t fileDescriptor)
{
    if (fileDescriptor < 0) {
...
void DaemonIoWaiter::DrainAwakenPipe() const
{
    uint64_t value = 0;
...
void DaemonIoWaiter::SetFileDescriptorEventCallback(const IoWaiter::FileDescriptorEventCallback &callback)
{
    callback_ = callback;
}

void DaemonIoWaiter::InsertFileDescriptorMap(int32_t fileDescriptor, const std::string& taskName,
...
void DaemonIoWaiter::EraseFileDescriptorMap(int32_t fileDescriptor)
{
    std::lock_guard<std::mutex> lock(fileDescriptorMapLock);
    fileDescriptorMap_.erase(fileDescriptor);
}

std::shared_ptr<FileDescriptorInfo> DaemonIoWaiter::GetFileDescriptorMap(int32_t fileDescriptor)
{
    std::lock_guard<std::mutex> lock(fileDescriptorMapLock);
    auto it = fileDescriptorMap_.find(fileDescriptor);
    if (it == fileDescriptorMap_.end()) {
        HILOGW("DaemonIoWaiter get file descriptor failed %{public}d", fileDescriptor);
        return nullptr;
    }
    return it->second;
}
