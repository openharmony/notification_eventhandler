    ...
    : EventQueue(ioWaiter), historyEvents_(std::vector<HistoryEvent>(HISTORY_EVENT_NUM_POWER))
{
    ...
    if (ioWaiter_->SupportListeningFileDescriptor()) {
        ...
        ioWaiter_->SetFileDescriptorEventCallback(
            std::bind(&EventQueueBase::HandleFileDescriptorEvent, this, std::placeholders::_1, std::placeholders::_2,
...
bool EventQueueBase::AddFileDescriptorByFd(int32_t fileDescriptor, uint32_t events, const std::string &taskName,
...
void EventQueueBase::HandleFileDescriptorEvent(int32_t fileDescriptor, uint32_t events,
...
bool EventQueueBase::EnsureIoWaiterSupportListerningFileDescriptorLocked()
{
    HILOGD("enter");
    if (ioWaiter_->SupportListeningFileDescriptor()) {
        return true;
...
void EventQueueBase::RemoveInvalidFileDescriptor()
{
